...
class NotificationController
{
    ...
    public function bookIndex(array $params): void
    {
        ...
        try {
            $notifs = Database::query(
                'SELECT * FROM notifications WHERE book_id=? AND user_id=? ORDER BY created_at DESC LIMIT 40',
                [$book['id'], auth()['id']]
            );
        } catch (\Throwable $e) {}
    ...
    public function send(array $params): void
    {
        ...
        if (!$title) redirect('/books/'.$book['id'].'/employees', ['error' => 'Notification title is required.']);
        ...
        if (empty($targets)) {
            redirect('/books/'.$book['id'].'/employees', ['error' => 'No recipients selected.']);
        }
        ...
        redirect('/books/'.$book['id'].'/employees',
    ...
    // HELPERS
    // =========================================================================
    public static function formatNotif(array $n): array
    {
        return [
            'id'         => $n['id'],
            'type'       => $n['type'],
