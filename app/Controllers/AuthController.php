...
class AuthController
{
    ...
    {
        $link = config('url') . '/verify-email?token=' . $token;
        $subject = 'Verify your Byabsayee account';
        $body = "Hi {$name},\n\nPlease verify your email by clicking this link:\n{$link}\n\nThis link does not expire.\n\n— Byabsayee";

        // We'll hook this up to PHPMailer when we set up the mail service
        // For now it logs to error_log so you can see the link during development
        error_log("VERIFICATION EMAIL to {$email}: {$link}");
        // TODO: replace with: MailService::send($email, $subject, $body);
    }
    ...
    {
        $link = config('url') . '/reset-password?token=' . $token;
        $subject = 'Reset your Byabsayee password';
        $body = "Hi {$name},\n\nClick this link to reset your password:\n{$link}\n\nExpires in 1 hour.\n\n— Byabsayee";

        error_log("RESET EMAIL to {$email}: {$link}");
        // TODO: replace with: MailService::send($email, $subject, $body);
    }
}
