...
class InvoiceController
{
    public function index(array $params): void
    {
        ...
        $status = $_GET['status'] ?? 'all';   // all, draft, sent, paid, overdue
    ...
    public function create(array $params): void
    {
        ...
        $suppliers = Database::query(
            'SELECT id, name, company FROM suppliers WHERE book_id=? AND deleted_at IS NULL ORDER BY name',
            [$book['id']]
        );
        $products = Database::query(
            'SELECT id, name, sell_price, buy_price, stock_qty, unit FROM products
             WHERE book_id=? AND deleted_at IS NULL ORDER BY name',
            [$book['id']]
        );
    ...
    public function store(array $params): void
    {
        ...
        if (empty($itemNames) || !array_filter($itemNames)) {
        ...
        foreach ($itemNames as $i => $itemName) {
            ...
            $subtotal += $lineTot;
            $items[]   = compact('itemName','qty','price','discPct','lineTot','pid');
        }
        ...
        Database::run(
            'INSERT INTO invoices
                (book_id,type,invoice_no,customer_id,supplier_id,date,due_date,
                 subtotal,discount,tax,total,paid,status,notes,created_by,created_at)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,0,?,?,?,?)',
            [
                $book['id'], $type, $invoiceNo, $customerId, $supplierId,
                $date, $dueDate ?: null, $subtotal, $discount, $tax, $total,
                'draft', $notes ?: null, auth()['id'], now()
            ]
        );
        $invoiceId = Database::lastId();

        // Insert line items + deduct stock for sales
        foreach ($items as $item) {
            Database::run(
                'INSERT INTO invoice_items (invoice_id,product_id,description,qty,unit_price,discount_pct,line_total)
                 VALUES (?,?,?,?,?,?,?)',
                [$invoiceId, $item['pid'], $item['itemName'], $item['qty'],
                 $item['price'], $item['discPct'], $item['lineTot']]
            );

            // Deduct stock when it's a sale invoice and linked to a product
            ...
            // Add stock when it's a purchase invoice
        ...
        Database::run(
            'UPDATE book_business_details SET invoice_counter = invoice_counter + 1 WHERE book_id=?',
            [$book['id']]
        );
    ...
    public function show(array $params): void
    {
        ...
        $customer = $invoice['customer_id']
            ? Database::row('SELECT * FROM customers WHERE id=?', [$invoice['customer_id']]) : null;
        $supplier = $invoice['supplier_id']
            ? Database::row('SELECT * FROM suppliers WHERE id=?', [$invoice['supplier_id']]) : null;
        $details  = Database::row('SELECT * FROM book_business_details WHERE book_id=?', [$book['id']]);
    ...
    public function recordPayment(array $params): void
    {
        ...
        if ($amount <= 0) {
            redirect('/books/'.$book['id'].'/invoices/'.$invoice['id'], ['error' => 'Amount must be greater than zero.']);
        }

        $due = $invoice['total'] - $invoice['paid'];
        if ($amount > $due) $amount = $due; // can't overpay
        ...
        // Add points to customer (1 point per 100 taka paid)
        if ($invoice['customer_id'] && $invoice['type'] === 'sale') {
            $points = (int)($amount / 100);
            if ($points > 0) {
                Database::run(
                    'UPDATE customers SET points = points + ? WHERE id=?',
                    [$points, $invoice['customer_id']]
                );
            }
        }
